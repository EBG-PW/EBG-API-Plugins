...
/* Plugin info*/
const PluginName = "EBG-serverstatus-C-Client";
...
router.get('/', limiter, async (reg, res, next) => {
	try {
		...
		var { StatsJson } = await getStats(`https://${process.env.ServerStatsURL}/api/v1/serverstatusCserver`)
		if(StatsJson != null) {
...
/**
 * Sends request to the serverstatusCserver.
 * @param {url} URI 
 */
